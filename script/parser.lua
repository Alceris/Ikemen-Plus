...
local name = ''
local batch = 'mkdir data\\charRes\nmkdir debug'
local t_gen = {}
t_bossChars = {}
t_bonusChars = {}
t_randomChars = {}
...
for i=1, #t_selChars do
	...
	if t_selChars[i].displayname ~= nil then
		...
		--if data/charRes/displayname.def doesn't exist
		if io.open('data/charRes/' .. displayname .. '.def','r') == nil then
			--create a batch variable used to create 'tools/wip/charName' folder and to extract all character's sprites there
			batch = batch .. '\n' .. 'mkdir "tools\\wip\\' .. displayname .. '"' .. '\n' .. 'tools\\sff2png.exe "' .. t_selChars[i].sff:gsub('/+', '\\') .. '" "tools\\wip\\' .. displayname .. '\\s"'
		...
		--otherwise load SFF file
			...
			t_selChars[i]['sffData'] = sffNew('data/charRes/' .. displayname .. '.sff')
...
if generate and data.sffConversion then
	...
	elseif parserCfg == 1 then
		...
		--open each line in tools/wip/charName/s-sff.def to compare
		for i=1, #t_gen do
			local append = ''
			displayname = t_selChars[t_gen[i]].displayname:gsub('%s+', '_')
			for line in io.lines('tools/wip/' .. displayname .. '/s-sff.def') do
			...
			f_printVar(sff2png .. append, 'data/charRes/' .. displayname .. '.def')
			--create a batch variable used to generate sff files all at once
			batch = batch .. '\n' .. 'tools\\sprmake2.exe' .. ' -o "data\\charRes\\' .. displayname .. '.sff" "data\\charRes\\' .. displayname .. '.def" >> debug\\sprmake2.log'
		end
		batch = batch .. '\n' .. 'del batch.bat'
		f_printVar(batch, 'batch.bat')
		os.execute('batch.bat')
		for i=1, #t_gen do
			displayname = t_selChars[t_gen[i]].displayname:gsub('%s+', '_')
			t_selChars[t_gen[i]]['sffData'] = sffNew('data/charRes/' .. displayname .. '.sff')
